            ...
            <div className="description">
              ...
              <Fade triggerOnce delay={4000}>
                <p style={{ maxWidth: 800 }}>
                  My passion for photography compels me to capture the essence of the world through
                  my lens, and I am dedicated to continually refining and perfecting my craft.
                </p>
              </Fade>
            </div>
        ...
        <div className="container inner-platforms">
          ...
          <ul className="platform-lists">
            {platforms.map((platform) => (
              <Fade key={platform.url} triggerOnce duration={1000} delay={200}>
                <li className="platform">
                  <Link to={platform.url} target="_blank">
                    {platform.icon}
                  </Link>
                </li>
              </Fade>
        ...
        <div className="container">
          ...
          <ul className="samples">
            {photos.arr.slice(0, photos.show).map((photo) => (
              <Fade key={photo.src} triggerOnce duration={1000} delay={200}>
                <li className="sample">
                  <span className="top">{photo.icon}</span>
                  <a className="thumbnail" href={photo.link} target="_blank" alt={photo.name}>
          ...
          <Fade triggerOnce duration={1000} delay={200}>
            {photos.isLoading ? (
              <Loading />
            ) : photos.show < photos.arr.length ? (
              <button className="load-more" onClick={() => handleLoadMore(setPhotos)}>
                Load More
              </button>
            ) : (
              ''
